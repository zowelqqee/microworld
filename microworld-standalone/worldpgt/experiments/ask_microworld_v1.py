...
_DEFAULT_COGNITIVE_PATTERNS_PATH = (
    _ROOT / "worldpgt" / "experiments" / "community_context_v1" / "cognitive_pattern_events.json"
)
...
def ask(
    ...
    orchestrator = AnswerOrchestrator(
        ...
        cognitive_patterns_enabled=cognitive_patterns_path is not None,
    )
...
def _run_interactive(
    *,
    ...
    cognitive_patterns_path: str | None,
    json_mode: bool,
...
def _resolve_cognitive_patterns_path(args, parser) -> str | None:
    if args.no_cognitive_patterns:
        return None
    candidate = (
        Path(args.cognitive_patterns)
        if args.cognitive_patterns
        else _DEFAULT_COGNITIVE_PATTERNS_PATH
    )
    if candidate.is_file():
        return str(candidate)
    if args.cognitive_patterns:
        parser.error(f"--cognitive-patterns does not exist or is not a file: {candidate}")
    return None
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional cognitive_pattern_events.json path. If omitted, the "
            "default community-context pattern artifact is used when it exists."
        ),
    )
    parser.add_argument(
    ...
    if args.interactive:
        return _run_interactive(
            ...
            overlay_path=overlay_path,
            ontology_layer_path=ontology_layer_path,
            cognitive_patterns_path=_resolve_cognitive_patterns_path(args, parser),
